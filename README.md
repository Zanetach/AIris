...
AIris 是一个面向 Obsidian 的侧边栏 AI 生图插件，核心目标是：
...
2. 解压到你的 Vault：
   `.obsidian/plugins/airis/`
3. 重启 Obsidian，在 `设置 -> 第三方插件` 启用 `AIris`。
...
AIris is a sidebar-first AI image plugin for Obsidian. The core flow is:
...
2. Extract to:
   `.obsidian/plugins/airis/`
3. Restart Obsidian and enable `AIris` in Community Plugins.
